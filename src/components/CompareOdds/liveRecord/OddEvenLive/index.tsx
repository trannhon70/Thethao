...
const OddEvenLive = ({ data, matchTime, keyStatus}) => {
  let dataFilter=[]
  if(keyStatus === "ht"){
  dataFilter = data?.overUnder?.[0]?.detail?.filter(item => moment.unix(item.modifyTime).diff(moment.unix(matchTime),'minutes') < 95)
  dataFilter= dataFilter?.filter(item => item?.modifyTime < matchTime)
  }else{
     dataFilter = data?.overUnder?.[0]?.detail?.filter(item => moment.unix(item.modifyTime).diff(moment.unix(matchTime),'minutes') < 95)

  }
  // let dataFilter = data?.overUnder?.[0]?.detail?.filter(item => moment.unix(item.modifyTime).diff(moment.unix(matchTime),'minutes') < 95)

  const odds1 = data?.overUnder?.[0]?.odds1;
  const odds2 = data?.overUnder?.[0]?.odds2;
  const odds3 = data?.overUnder?.[0]?.odds3;
  const columns = [
    {
      title: "Tỉ lệ tài xỉu",
      children: [
  ...
  return (
    <>
      <Table
        // dataSource={data?.overUnder?.[0]?.detail}
